(async () => {
  ...
  function renderList() {
    $('#short-library-status').textContent=listError?t(listError):loaded?'':t('loading');
  ...
  function setBusy(value) {busy=value;form.setAttribute('aria-busy',String(value));form.querySelectorAll('input,button').forEach(el=>{el.disabled=value;});$('#confirm-delete-short').disabled=value;$('#cancel-delete-short').disabled=value;$('#new-short-link').disabled=value;}
  ...
  $('#new-short-link').addEventListener('click',()=>{if(!busy)select(null);});
