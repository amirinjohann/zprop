...
vCards are counted when generated for download. `POST /api/vcards` stores only a content fingerprint under the account in private `.created-vcards/`; contact fields stay in the browser. Repeated downloads of identical content count once; changed content counts as a new card. Earlier downloads cannot be recovered. Back up `.created-vcards/` with the other private storage directories. Counts represent created items, not visits or download completions.
