window.ZpropItemLibrary={mount({category,onCreate,onEdit,onDownload}){
  ...
  const urlFor=item=>new URL(item.url,/^(localhost|127\.0\.0\.1|\[::1\])$/.test(location.hostname)?location.origin:window.ZPROP_PUBLIC_ORIGIN).href;
  ...
  function render(){
    $('#item-library-status').textContent=error?t(error):loaded?'':t('loading');
  ...
  $('#new-item').addEventListener('click',()=>{if(!isBusy())select(null);});
  ...
  $('#confirm-delete-item').addEventListener('click',async()=>{
    ...
    finally{busy=false;$('#confirm-delete-item').disabled=false;$('#cancel-delete-item').disabled=false;}
  });
