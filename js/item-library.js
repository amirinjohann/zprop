window.ZpropItemLibrary={mount({category,onCreate,onEdit,onDownload}){
  ...
  const names={
    'transfer-files':{title:['Pautan fail anda','Your file links'],create:['Cipta pautan fail','Create file link'],empty:['Belum ada pautan fail','No file links yet'],help:['Cipta, edit dan urus pautan fail anda di sini.','Create, edit and manage your file links here.'],start:['Pilih Cipta pautan fail untuk menambah fail pertama anda.','Choose Create file link to add your first file.'],icon:'\u2197'},
  ...
  function render(){
    ...
    $('#item-list').innerHTML=error?`<button type="button" id="retry-items" class="short-primary">${t('retry')}</button>`:items.map(item=>`<article class="short-link-card" data-item-id="${esc(item.id)}"><div><span class="short-badge">${t(item.status)}</span><h3>${esc(item.name)}</h3>${item.url?`<a href="${esc(urlFor(item))}" target="_blank" rel="noopener noreferrer">${esc(urlFor(item))}</a>`:!item.manageUrl?`<p class="short-hint">${t('legacy')}</p>`:''}</div><div class="tool-actions">${item.url?`<a href="${esc(urlFor(item))}" target="_blank" rel="noopener noreferrer" data-item-action="open">${t('open')}</a><button type="button" data-item-action="copy">${t('copy')}</button>`:''}${(category==='vcards'&&item.manageUrl)||category==='transfer-files'||category==='host-html'?`<button type="button" data-item-action="edit">${t('edit')}</button>`:''}${category==='vcards'&&item.manageUrl?`<button type="button" data-item-action="download">${t('download')}</button>`:''}<button type="button" data-item-action="delete" class="short-danger">${t('remove')}</button></div></article>`).join('');
  ...
  const itemApi={vcards:id=>'/api/vcards/'+encodeURIComponent(id),'transfer-files':id=>'/api/file-links/'+encodeURIComponent(id),'host-html':id=>'/api/static-sites/'+encodeURIComponent(id)};
  function clean(record=active){active=record;dirty=false;baseline=snapshot();$('#item-dirty').hidden=true;$('#item-active-name').textContent=record?.state?.name||record?.filename||record?.name||record?.slug||'';}
  function select(record){if(record)onEdit?.(record);else onCreate();clean(record);$('#item-library').hidden=true;editor.hidden=false;query(record?.id);[...form.elements].find(el=>el.tagName!=='BUTTON'&&el.type!=='hidden'&&!el.readOnly&&el.offsetParent)?.focus();}
  const canLeave=()=>!isBusy()&&(!dirty||window.confirm(t('discard')));
  async function open(id){if(!canLeave())return;busy=true;try{select(await api(itemApi[category](id)));}catch(e){error=copy[e.message]?e.message:'error';render();}finally{busy=false;}}
  ...
  refresh();connect();
  const requested=new URL(location.href).searchParams.get('item');if(requested)open(requested);
