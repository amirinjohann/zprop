(async () => {
  ...
  const copy = {
    ...
    imageError:['Pilih imej PNG, JPG, WEBP atau GIF yang sah, sehingga 5 MB.','Choose a valid PNG, JPG, WEBP or GIF image, up to 5 MB.'], imageTotal:['Jumlah imej maksimum ialah 20 MB. Buang imej atau pilih fail lebih kecil.','Images can total up to 20 MB. Remove an image or choose a smaller file.'], imageMissing:['Tambah imej pada setiap blok imej dahulu.','Add an image to each image block first.'], imageLoading:['Sila tunggu imej selesai dimuatkan.','Please wait for your images to finish loading.'], blockLimit:['Anda boleh menambah sehingga 30 blok.','You can add up to 30 blocks.'], invalidUrl:['Masukkan URL lengkap bermula dengan https:// atau http://.','Enter a complete URL starting with https:// or http://.'], taken:['Nama pautan ini sudah digunakan. Pilih nama lain.','This link name is already taken. Choose another name.'], itemLimit:['Anda boleh menyimpan sehingga 5 item untuk alatan ini. Padam satu untuk menambah yang baharu.','You can save up to 5 items in this tool. Delete one to add another.'], server:['Halaman tidak dapat diterbitkan. Sila cuba lagi.','Could not publish your page. Please try again.'], busy:['Pelayan sedang sibuk. Cuba lagi sebentar.','The server is busy. Please try again shortly.'], slugError:['Gunakan 3–50 huruf kecil atau nombor, dengan sempang di tengah sahaja.','Use 3–50 lowercase letters or numbers, with hyphens only in the middle.'], downloadError:['Muat turun tidak dapat disediakan. Sila cuba lagi.','Could not prepare your download. Please try again.']
  };
  Object.assign(copy, {
  ...
  function customHtml(block) {
    ...
    const source = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src https: data:; font-src https: data:; script-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'"><style>html{color:${state.ink};font-family:Arial,sans-serif;overflow-wrap:anywhere}body{margin:0}img{max-width:100%;height:auto}a{color:inherit}</style></head><body>${block.html || ''}</body></html>`;
  ...
  function documentHtml() { const profile = state.blocks.find(block => block.type === 'profile' && block.enabled !== false); return `<!DOCTYPE html><html lang="${language?'en':'ms'}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="description" content="${esc(profile?.bio || '')}"><title>${esc(profile?.name || library.active?.slug || 'Bio page')} — ZPROP</title><style>${pageStyles}${layoutStyles}html{height:100%;background:${state.background};overflow-x:clip;-webkit-text-size-adjust:100%}body{margin:0;min-height:100%;min-height:100dvh;padding:env(safe-area-inset-top,0) env(safe-area-inset-right,0) env(safe-area-inset-bottom,0) env(safe-area-inset-left,0);${variables()}}.bio-document{max-width:520px;margin:auto;min-height:100vh;min-height:100dvh}</style></head><body><main class="bio-document" data-layout="${state.layout || 'classic'}">${markup()}</main></body></html>`; }
